...
export default async function FaqPage() {
  ...
  const grouped = faqs.reduce<Record<string, Faq[]>>((result, faq) => {
    const category = faq.categoryName ?? "Umumiy savollar";
  ...
  return <main style={{ maxWidth: 980, margin: "0 auto", padding: "32px 20px 90px" }}>
    ...
    {Object.entries(grouped).map(([category, items]) => <section key={category} style={{ marginBottom: 42 }}><h2>{category}</h2>{items.map((faq) => <details key={faq.id} style={{ padding: "20px 0", borderBottom: "1px solid #dce8e0" }}><summary style={{ fontSize: 19, fontWeight: 700, cursor: "pointer" }}>{faq.question}</summary><div style={{ paddingTop: 14, lineHeight: 1.7 }}><p>{faq.fullAnswer || faq.shortAnswer}</p>{faq.relatedArticleSlug && <Link href={`/maqolalar/${faq.relatedArticleSlug}`}>Batafsil maqolani o‘qish →</Link>}</div></details>)}</section>)}
