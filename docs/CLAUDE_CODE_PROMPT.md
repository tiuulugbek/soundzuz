...
2. **Server-side (SSR) fetch ichki API'ga.** `apps/web/lib/*.ts` fetch'lari `lib/api-url.ts` dagi `serverApiUrl()` ni ishlatadi (`API_INTERNAL_URL` = `http://api:4000/v1`). Media URL (brauzer uchun) `toMediaUrl()`/`browserApiUrl()`. Yangi lib yozsang shu helper'lardan foydalan — env'ni qo'lda o'qima.
3. **Nginx 502.** Web konteyner qayta yaratilganda yangi IP oladi; Nginx statik `upstream`da eski IP'ni keshlab 502 beradi. Yechim `deploy/nginx/gateway/default.conf` da: `resolver 127.0.0.11` + `proxy_pass http://$o'zgaruvchi`. Bu papka konteynerga PAPKA sifatida mount qilinadi (bitta fayl mount qilinsa, faylni qayta yozish inode'ni almashtiradi va konteyner eski nusxada qolib ketadi). Deploy `scripts/deploy-zero-downtime.sh` bilan (pull → migrate → `up -d` recreate → gateway reload → tekshiruv → rollback).
4. **Brauzer API manzili nisbiy (`/v1`).** `NEXT_PUBLIC_*` build vaqtida bundle'ga singdiriladi — serverdagi `.env` ularni o'zgartira olmaydi, `.github/workflows/container.yml` build-args'i hal qiladi. Server tomonda esa nisbiy manzil ishlamaydi: `lib/api-url.ts` dagi `serverApiUrl()` faqat absolut manzil qaytaradi (aks holda build prerender'da osilib qoladi).
...
6. **Mavjud kontent jadvallari.** `content_hub` migratsiyasi allaqachon `articles`, `article_categories`, `faqs`, `faq_categories` yaratgan (ContentStatus enum bilan). Ularni QAYTA yaratma — ustiga qur. `article_categories`/`faq_categories` slug endi `(slug, locale)` unique.
7. **Migratsiyalar.** Qo'lda, idempotent SQL (`CREATE TABLE IF NOT EXISTS`, `ALTER ... IF EXISTS`), `packages/database/prisma/migrations/<timestamp>_<nom>/migration.sql`. `prisma migrate deploy` bilan qo'llanadi.
8. **Dizayn tizimi.** Tokenlar `apps/web/app/tokens.css` (`--sz-orange #FF8201`, `--sz-dark #252423`, iliq yuzalar, `--sz-*`). Komponentlar `apps/web/components/ui` (Button, Section, Reveal, Marquee, WaveCanvas...). Class nomlari `sz-*`. Har sahifa `SiteHeader`/`SiteFooter`, `buildPageMetadata` (SEO+hreflang), `components/seo/json-ld` (Product/Article/FAQPage/Breadcrumb/LocalBusiness/MedicalWebPage).
9. **Namuna seed.** `pnpm db:seed:catalog` — katalog (3 brend, 9 model, xizmat, namuna filial) + Content Hub (10 kategoriya, 14 maqola, FAQ). Idempotent, admin'ga tegmaydi. Fayllar: `packages/database/prisma/catalog-sample.ts`, `content-sample.ts`, ishga tushirgich `seed-catalog-run.ts`. Namuna ma'lumot vaqtinchalik — asoschi admin orqali haqiqiysi bilan almashtiradi.
